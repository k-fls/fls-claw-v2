...
describe('host lifecycle orchestration', () => {
  ...
  it('aborts modules and awaits their LIFO shutdown before host cleanup', () => {
    // Our fork's shutdown orchestration lives in src/shutdown.ts (initiateShutdown),
    // not inline in index.ts signal handlers. The abort() call is in index.ts;
    // the ordered stop calls are in shutdown.ts.
    const indexSource = fs.readFileSync(path.join(process.cwd(), 'src', 'index.ts'), 'utf8');
    const shutdownSource = fs.readFileSync(path.join(process.cwd(), 'src', 'shutdown.ts'), 'utf8');
    const abort = indexSource.indexOf('hostAbortController.abort()');
    const modulesStop = shutdownSource.indexOf('await stopHostModules()');
    const pollsStop = shutdownSource.indexOf('stopDeliveryPolls()');

    expect(abort).toBeGreaterThan(-1);
    expect(modulesStop).toBeGreaterThan(-1);
