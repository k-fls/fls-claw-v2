...
// LOAD-BEARING: `migrateGroupsToClaudeLocal` recognises a generated project
// document by this prefix and declines to promote it into a group's memory. It
// must stay the literal first characters of the file for every provider;
// otherwise that migration renames the whole runtime contract to
// `CLAUDE.local.md`. `project-doc-compose.test.ts` ("starts with the
// composed-at-spawn marker") goes red if it moves.
export const COMPOSED_DOC_PREFIX = '<!-- Composed at spawn';

const COMPOSED_HEADER = `${COMPOSED_DOC_PREFIX} - do not edit. Standing instructions: instructions.prepend.md. Memory: memory/. -->`;

const BASE_DOC_SECTION = 'NanoClaw Runtime Contract';

// Instruction docs that may be pure `ncl` teaching, and so dead weight when the
// agent has none: host dispatch rejects every cli_request at cli_scope=disabled.
// Whether a candidate actually depends on ncl differs between editions — a
// module that is an ncl surface on one is a standalone MCP tool on another — so
// membership is confirmed against the doc's own text rather than assumed.
const NCL_DEPENDENT_CANDIDATES = new Set(['cli', 'scheduling']);
const NCL_REFERENCE = /\bncl\b/;

export function isNclDependent(moduleName: string, body: string): boolean {
  return NCL_DEPENDENT_CANDIDATES.has(moduleName) && NCL_REFERENCE.test(body);
}
 ...
 *
 * Reads nothing the agent can author except `instructions.prepend.md`.
 */
export async function composeGroupProjectDoc(group: AgentGroup, groupDir: string, spec: ProjectDocSpec): Promise<void> {
  ...
  const mcpServers = sanitizeStoredMcpServers(configRow?.mcp_servers ?? {}, group.name);
  ...
  const baseBody = fs.existsSync(baseDoc) ? fs.readFileSync(baseDoc, 'utf-8') : '';
  if (baseBody.trim()) {
    push(BASE_DOC_SECTION, baseBody);
  } else {
    ...
    // looks like. An empty file is indistinguishable from a missing one at the
    // agent, so both warn.
  ...
  if (fs.existsSync(mcpToolsHostDir)) {
    for (const entry of fs.readdirSync(mcpToolsHostDir).sort()) {
      ...
      const body = fs.readFileSync(path.join(mcpToolsHostDir, entry), 'utf-8');
      if (cliDisabled && isNclDependent(moduleName, body)) continue;
      push(`NanoClaw Module: ${moduleName}`, body, true);
    }
  }
