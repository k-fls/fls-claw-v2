...
// tests below. (roles-grant is registered above for the approval-card suite.)
register({
  name: 'roles-revoke',
...
describe('role-change approval routing', () => {
  ...
  //
  // Adapted to main_patched's grant-carrying replay: the cli_command handler
  // re-enters dispatch as the ORIGINAL caller with the verified approval row as
  // its grant (not bare host). The apply-time approver re-check runs BEFORE the
  // replay, so the two rejection cases return before the grant is ever used;
  // the success case exposes a live grant via getPendingApproval so the guard
  // re-check passes.
