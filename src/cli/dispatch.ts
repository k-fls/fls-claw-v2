...
export async function dispatch(
  ...
  if (decision.effect === 'hold') {
    ...
    // main_patched propagates the caller context on the pending row so the
    // approved replay re-enters dispatch as the original agent (not bare host).
 ...
 *
 * C1/C4 seam (initiator principal / honor-authority): main_patched DOES
 * propagate a CallerContext through the approval replay, but for an agent
 * caller it carries only the session/agent-group identity — NOT a verified
 * human principal — and an agent must not be trusted to self-assert one. So we
 * do NOT auto-approve owner-authorized changes here (C4 stays deferred). If a
 * verified initiating human principal is ever plumbed into CallerContext, add
 * it to `excludeUserIds` (never approve your own change) and short-circuit when
 * the principal already holds privilege >= the change.
 */
