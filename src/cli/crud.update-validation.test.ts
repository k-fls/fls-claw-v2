...
describe('genericUpdate flag validation (#6)', () => {
  ...
  it('rejects a non-updatable column (agent_group_id) instead of silently ignoring it', async () => {
    ...
    const row = await getDb().get<{ agent_group_id: string }>(
      'SELECT agent_group_id FROM messaging_group_agents WHERE id = ?',
      WID,
    );
  ...
  it('still applies a legitimate update (does not over-reject)', async () => {
    ...
    const row = await getDb().get<{ engage_mode: string; engage_pattern: string }>(
      'SELECT engage_mode, engage_pattern FROM messaging_group_agents WHERE id = ?',
      WID,
    );
