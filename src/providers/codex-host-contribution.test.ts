...
describe('codex host contribution against real core', () => {
  beforeEach(async () => {
    ...
    // Fork-added: this edition's `buildMounts` pulls its launch shape from
    // container-bootstrap's snapshot, which upstream has no notion of, so the
    // snapshot must exist before it is called. cwd is the repo root here, which
    // has the real container/ tree.
