...
async function spawnContainer(session: Session): Promise<void> {
  ...
  const mounts = buildMounts(agentGroup, session, containerConfig, provider, contribution, groupContribution);
...
export interface ComposeSessionSpecInput {
  ...
  /** Per-agent-group dynamic contribution (env vars from registered callbacks). Mounts are already folded into `mounts` by `buildMounts`. */
  groupContribution?: AgentGroupContribution;
  /**
...
export function composeSessionSpec(input: ComposeSessionSpecInput): SessionSpec {
  const { agentGroup, session, containerName, mounts, containerConfig, contribution, groupContribution, gateway } = input;
  ...
  // Fill order: provider → per-agent-group callbacks → gateway (gateway wins
  // key collisions, the same override the old raw-argv append got from
  // Docker's last-wins rule).
  const contributedEnv: Record<string, string> = {
    ...(contribution.env ?? {}),
    ...(groupContribution?.env ?? {}),
    ...(gateway.env ?? {}),
  };
