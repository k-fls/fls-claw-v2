...
import type { MergedSpawnPre } from './modules/container-bootstrap/index.js';

// Inert container-bootstrap pre-spawn merge — these tests exercise buildMounts'
// surface/mount logic, not the bootstrap lifecycle.
const EMPTY_SPAWN_PRE: MergedSpawnPre = { mounts: [], env: {}, args: [], needsRootEntrypoint: false, cleanups: [] };
...
import { initSnapshot, __resetSnapshotForTests } from './modules/container-bootstrap/snapshot.js';
...
beforeEach(() => {
  ...
  // buildMounts' launch shape + /app/CLAUDE.md surface are snapshot-derived
  // (container-bootstrap), so a snapshot must exist. cwd is the repo root here,
  // which has the real container/ tree.
...
describe('buildMounts agent surfaces', () => {
  ...
  it('suppresses the default surfaces and keeps contributed mounts for a surfaces-providing provider', () => {
    ...
    const mounts = buildMounts(ag, session('s2', ag.id), containerConfig(), 'surfaces-test-provider', contributed, {}, EMPTY_SPAWN_PRE);
