...
import { defaultLaunchShape, snapshotAgentSurfaces } from './launch-shape.js';
...
describe('defaultLaunchShape', () => {
  it('mounts entrypoint, agent-runner src, skills (infra) from the snapshot — all read-only', () => {
    initSnapshot();
    ...
    // CLAUDE.md is an agent *surface*, not launch infra — it moved out.
    expect(byContainer.has('/app/CLAUDE.md')).toBe(false);
  });

  it('skips infra mounts whose source file is absent in the snapshot', () => {
...
describe('snapshotAgentSurfaces', () => {
  it('mounts /app/CLAUDE.md from the snapshot — read-only', () => {
    initSnapshot();
    const byContainer = new Map(snapshotAgentSurfaces().map((m) => [m.containerPath, m]));
    expect(byContainer.get('/app/CLAUDE.md')).toMatchObject({
      hostPath: snapshotPath('CLAUDE.md'),
      readonly: true,
    });
  });

  it('skips /app/CLAUDE.md when absent from the snapshot', () => {
    fs.rmSync(path.join(process.cwd(), 'container', 'CLAUDE.md'));
    initSnapshot();
    expect(snapshotAgentSurfaces().map((m) => m.containerPath)).not.toContain('/app/CLAUDE.md');
  });
});
