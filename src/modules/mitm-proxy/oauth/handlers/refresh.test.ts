...
function buildCtx(opts: {
  seed?: Array<{ scope: CredentialScope; cred: Credential }>;
...
describe('tryRefresh — borrowed credential (owningScope)', () => {
  ...
  it('dedups two borrowers of one grantor into a single exchange (keyed by owning scope)', async () => {
    ...
    const { ctx } = buildCtx({ seed: [{ scope: GRANTOR, cred: cred('GRANTOR_ACCESS', 'GRANTOR_REFRESH') }], fetchImpl });
    const provider = buildProvider();

    const results = await Promise.all([
      tryRefresh(provider, b1, ctx, GRANTOR),
      tryRefresh(provider, b2, ctx, GRANTOR),
    ]);
