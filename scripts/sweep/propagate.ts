...
async function prepareCaseMaterials(cli: Cli, dir: string, rc: ResolvedCase, tier: Tier): Promise<string> {
  ...
  const materials = [
    ...
    '',
    'RESOLVE ONLY these conflicted paths by editing out the conflict markers, and write',
    'the PR text from the two sides below + this brief. Do not open or read any other',
    'file; do not explore the repo — the driver already did the analysis (D-056). If',
    'the two sides + this brief are not enough to decide, use `--tier held`.',
    '',
    '## Conflicted paths',
...
async function machineCaseMaterials(cli: Cli, jc: JournaledCase): Promise<string> {
  ...
  return [
    ...
    '',
    'RESOLVE ONLY these conflicted paths by editing out the conflict markers. Do not',
    'open or read any other file; do not explore the repo — the driver already did the',
    'analysis (D-056). If the two sides below + this brief are not enough to decide,',
    'use `--tier held`.',
    '',
    '## Conflicted paths',
