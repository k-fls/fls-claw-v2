...
ERR47_ENVIRONMENT_UNUSABLE — dependencies would not install into a tree whose manifests are committed and valid, so the machine is broken and nothing in the fork can fix it. No case is served into such a tree and no check run in one is evidence about the code. Do NOT edit code, do NOT retry, and do NOT work around it. REPORT AND STOP, naming the command and the output the detail carries.
