...
## The Credential-Proxy Complication

NanoClaw normally runs API calls through the host-side MITM credential proxy that injects real credentials in place of a substitute token. When redirecting to Ollama you need to bypass that proxy so requests go direct. Two env vars handle this:
...
**`src/container-runner.ts`** — At container spawn time, `env` entries become `-e KEY=VAL` Docker flags (applied after the credential proxy's injected vars so they win), and `blockedHosts` entries become `--add-host HOST:0.0.0.0` flags.
