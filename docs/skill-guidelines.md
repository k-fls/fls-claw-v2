...
5. **Credential threading** (`-e KEY=…` or a stdin secrets payload into the container). Use the MITM credential proxy only; it swaps substitute tokens for real credentials per request. Never pass raw secrets into the container.
